...
// HandlerInterceptor는 인터페이스지만, 내부함수가 defalut 키워드로 처리되어 있다
...
public class LoginInterceptor implements HandlerInterceptor {
	...
	// 1. preHandle 		: 요청이 컨트롤러의 함수(Handler)에 도달하기 전에 작동한다 (반환형: boolean -> 이 이후 컨트롤러가 진행될지 말지를 결정해야 하므로)
	// 2. postHandle 		: 컨트롤러의 함수(Handler)가 실행된 이후 view(jsp)로 넘어가기 전에 작동한다	(반환형 : void -> 이미 컨트롤러가 실행되었기 때문)
	// 3. afterCompletion 	: view(jsp)까지 모두 실행된 이후 작동한다	
	
	@Override
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
			throws Exception {
		// Handle : 컨트롤러의 실행함수를 뜻한다 -> preHandle 컨트롤러가 실행되기 전에 실행되는 함수!
		...
		if(session.getAttribute("login") == null) {	// 만약 세션에 로그인이 없다면
		...
		return true;	// true 반환시 컨트롤러 함수 그대로 진행
	}					// false 반환시 컨트롤러 실행 X
	...
	public void postHandle(HttpServletRequest request, HttpServletResponse response, Object handler,
		...
		// 따라서 이런 경우에는 header.jsp를 활용하기 보다는 postHandle을 사용한다
